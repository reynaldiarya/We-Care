    ...
    <div class="row justify-content-center p-4">
        @foreach ($transaksi as $item)
            <div class="card hvr-grow m-2"
                ...
                <div class="card-body">
                    <a href="campaign/{{ $item->campaign['slug_campaign'] }}" class="card-title" style="text-decoration: none; color:black">
                    ...
                    <p class="card-text mb-1">Status : @if($item->status_transaksi == 1)Berhasil @elseif($item->status_transaksi == 0)Belum Dibayar @elseif($item->status_transaksi == 2)Kadaluarsa @elseif($item->status_transaksi == 3)Dibatalkan @endif </p>
                    @if($item->status_transaksi == 0)
                    <a href="/checkout/{{ $item->id }}"><button class="btn" style="background-color:#435ebe; color:#fff">Bayar Sekarang</button></a>
                    @endif
                </div>
            </div>
        @endforeach
